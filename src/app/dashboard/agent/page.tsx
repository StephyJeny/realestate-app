...
export default function AgentDashboard() {
    ...
    const handleVerifyCode = async () => {
        if (!user || !verifyCode) return;
        try {
            const success = await verifyAgentCode(user.uid, verifyCode);
            if (success) {
        ...
        <div className={styles.dashboardPage}>
            ...
            <main className={styles.mainContent}>
                ...
                {isPending && (
                    <div className={styles.pendingBanner}>
                        ...
                        <div className={styles.pendingBannerContent}>
                            ...
                            <p>Your agent application is being reviewed by our admin team. You&apos;ll receive a notification with your verification code once approved.</p>
                        </div>
                    </div>
                )}

                {isRejected && (
                    <div className={styles.pendingBanner} style={{ borderColor: "rgba(239,68,68,0.2)", background: "rgba(239,68,68,0.06)" }}>
                        <div className={styles.pendingBannerIcon} style={{ background: "rgba(239,68,68,0.12)", color: "var(--error)" }}>❌</div>
                        <div className={styles.pendingBannerContent}>
                ...
                {isApproved && !isVerified && (
                    <div className={styles.pendingBanner} style={{ borderColor: "rgba(16,185,129,0.2)", background: "rgba(16,185,129,0.06)" }}>
                        <div className={styles.pendingBannerIcon} style={{ background: "rgba(16,185,129,0.12)", color: "var(--success)" }}>🔑</div>
                        <div className={styles.pendingBannerContent}>
                            <h3>Enter Your Verification Code</h3>
                            <p>Your application has been approved! Check your notifications for the verification code and enter it below.</p>
                            <div className={styles.codeInputGroup}>
                                <input
                                    ...
                                    maxLength={9}
                                />
                                <button className={styles.codeSubmitBtn} onClick={handleVerifyCode}>Verify</button>
                            </div>
                        </div>
                    </div>
                )}
                    ...
                    <>
                        <div className={styles.statsGrid}>
                            ...
                            <div className={styles.statCard}>
                                ...
                                <div className={styles.statValue}>{isVerified ? "Verified" : isPending ? "Pending" : isRejected ? "Rejected" : "Approved"}</div>
                            ...
                            {showEditProfile ? (
                                ...
                                <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "1rem" }}>
                                    ...
                                    <div><strong style={{ fontSize: "0.78rem", color: "var(--text-tertiary)", display: "block" }}>Status</strong>
                                        <span className={`${styles.statusBadge} ${isVerified ? styles.statusApproved : isPending ? styles.statusPending : styles.statusRejected}`}>
                                            {isVerified ? "Verified" : isPending ? "Pending" : isRejected ? "Rejected" : "Approved"}
                                        </span>
                                    </div>
